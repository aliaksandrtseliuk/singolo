...
const onScroll = () => {
  ...
  sections.forEach((block) => {
    if (
      block.offsetTop <= curPos &&
      block.offsetTop + block.offsetHeight > curPos
    ) {
      links.forEach((link) => {
        link.classList.remove("navigation__link--current");
        if (
          block.getAttribute("id") === link.getAttribute("href").substring(1)
        ) {
...
// Изменение вида HEADER при scroll
  ...
  if (window.pageYOffset >= 50) {
    ...
    document
      ...
      .classList.add("header__burger--onScroll");
  } else {
    document.querySelector(".header").classList.remove("header--scroll");
    document
      .querySelector(".navigation__link")
      .classList.add("navigation__link--current");
    document
      .querySelector(".header__burger")
      .classList.remove("header__burger--onScroll");
  }
};
...
const phoneHorizontal = document.querySelector(
  ".slider__image--phone-horizontal"
);
const phoneImageVertical = document.querySelector(
  ".slider__phone-background--vertical"
);
const phoneImageHorizontal = document.querySelector(
  ".slider__phone-background--horizontal"
);

phoneVertical.addEventListener("click", () =>
  phoneImageVertical.classList.toggle("slider__image--hidden")
);
phoneHorizontal.addEventListener("click", () =>
  phoneImageHorizontal.classList.toggle("slider__image--hidden")
);
...
const activeButton = (event) => {
  porfolioButtons.forEach((button) =>
    button.classList.remove("portfolio__button--current")
  );
...
document
  ...
  .addEventListener("click", activeButton);
...
document
  .querySelector(".form-quote__button")
...
document.querySelectorAll(".slider__button").forEach((item) =>
  item.addEventListener("click", () => {
    document
      ...
      .classList.toggle("slider--slide-background");
  })
);
...
const toggleLeftNav = () => {
  ...
  document
    ...
    .classList.toggle("header__link--left-slide");
  document
    .querySelector(".navigation")
    .classList.toggle("navigation--left-slide");
};
...
navLinks.forEach((link) =>
  link.addEventListener("click", () => {
    if (
      document
        .querySelector(".navigation")
        .classList.contains("navigation--left-slide")
    ) {
      toggleLeftNav();
    }
  })
